...
serve(async (req) => {
  ...
  try {
    ...
    if (q) {
      const searchUrl = `https://query1.finance.yahoo.com/v1/finance/search?q=${encodeURIComponent(q)}&lang=en-IN&region=IN&quotesCount=6&newsCount=0`
    ...
    if (!ticker) throw new Error('Missing ticker parameter or search query')
