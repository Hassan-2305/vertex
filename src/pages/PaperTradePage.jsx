...
export default function PaperTradePage() {
  ...
  return (
    <div className={`mx-auto fade-in h-full relative ${viewMode === 'terminal' ? 'p-0 flex flex-col' : 'p-6 max-w-[1100px]'}`}>
      ...
      {/* Top Header */}
      <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 ${viewMode === 'terminal' ? 'px-4 sm:px-6 py-4 bg-[#111118] border-b border-white/[0.07] z-10 relative' : ''}`}>
      ...
      {viewMode === 'terminal' ? (
        <div className="flex-1 px-4 pb-4">
          <TerminalTab
            onTradeAdded={() => queryClient.invalidateQueries({ queryKey: ['paper_trades', user.id] })}
