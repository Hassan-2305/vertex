...
export default function AuthPage() {
  ...
  // Reset loading if user comes back after a Google OAuth redirect
              ...
              <button 
                onClick={async () => {
                  ...
                  try {
                    ...
                    if (error) throw error
                    // Reset loading after 2 seconds if redirect is blocked/delayed
                    setTimeout(() => {
                      setLoading(false)
                    }, 2000)
                  } catch (err) {
                    ...
                    if (msg.includes('Unsupported provider') || msg.includes('provider is not enabled')) {
                      setError('Google Sign-In is not enabled. Please enable Google provider in your Supabase Dashboard under Authentication > Providers.')
                    } else {
                      setError(msg)
                    }
                    setLoading(false)
...
function GhostCandlesticks() {
  // useMemo prevents regenerating random positions on every parent re-render
