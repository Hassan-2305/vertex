...
export function useSymbolSearch(query) {
  ...
  useEffect(() => {
    ...
    const search = async () => {
      try {
        const searchUrl = `https://query1.finance.yahoo.com/v1/finance/search?q=${encodeURIComponent(debouncedQuery)}&lang=en-IN&region=IN&quotesCount=6&newsCount=0`
        const res = await fetch(`https://corsproxy.io/?url=${encodeURIComponent(searchUrl)}`, { signal: ctrl.signal })
