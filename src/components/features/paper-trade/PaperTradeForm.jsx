...
export default function PaperTradeForm({ onSubmit, isSubmitting, availableCapital }) {
  ...
  const orderCost = Number(entryPrice) * Number(quantity)
  const overLimit = availableCapital !== undefined && orderCost > availableCapital && orderCost > 0
  ...
  return (
    <div className="bg-[#111118] border border-white/[0.07] rounded-xl p-5">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-3">
        ...
        {/* Margin Preview */}
        {entryPrice && quantity ? (
          <div className={`border rounded-lg py-3 px-4 relative overflow-hidden transition-colors ${overLimit ? 'bg-[rgba(247,97,79,0.08)] border-[rgba(247,97,79,0.3)]' : 'bg-[#0a0a0f] border-white/[0.07]'}`}>
            <div className={`absolute left-0 top-0 bottom-0 w-[2px] ${overLimit ? 'bg-[#f7614f]' : 'bg-[#4f8ef7]'}`} />
            <div className="flex justify-between items-center pl-2">
              <span className="text-text-muted text-[10px] font-bold uppercase tracking-wider">Margin Required</span>
              <span className={`font-mono font-bold text-[14px] ${overLimit ? 'text-[#f7614f]' : 'text-white'}`}>{INR(orderCost)}</span>
            </div>
            {overLimit && (
              <div className="pl-2 mt-1.5 text-[10px] text-[#f7614f] font-medium">
                ⚠ Insufficient capital — available: {INR(availableCapital)}
              </div>
            )}
          </div>
        ) : null}
        ...
        <button 
          ...
          className={cn(
            "w-full py-3 rounded-lg text-[14px] font-semibold transition-all duration-150 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed",
            tradeType === 'buy'
