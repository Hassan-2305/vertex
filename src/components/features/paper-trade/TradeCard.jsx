...
export default function TradeCard({ trade, ltp: propLtp, onClose }) {
  const ltp = propLtp
  const [exitPrice, setExitPrice] = useState(ltp ? String(ltp) : '')
      ...
      {trade.status === 'open' && (
        <div className="flex gap-2 items-center bg-[#0a0a0f] p-2.5 rounded-lg border border-white/[0.07]">
          ...
          {ltp && (
            <button
              type="button"
              onClick={() => setExitPrice(String(ltp))}
              className="px-2 py-2 text-[10px] font-bold rounded-md bg-[rgba(79,142,247,0.1)] border border-[rgba(79,142,247,0.25)] text-[#4f8ef7] hover:bg-[rgba(79,142,247,0.2)] transition-colors whitespace-nowrap"
              title="Fill with live price"
            >LTP</button>
          )}
          {pnlEstimate !== null && (
            <span className={cn(
