...
// Fetch live price from Yahoo Finance via Supabase Edge Function
async function fetchLivePrice(symbol) {
  try {
    ...
    const res = await fetch(
      `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/yfinance-proxy?ticker=${encodeURIComponent(ticker)}&period1=${p1}&period2=${p2}`,
      {
...
async function searchWithQuotes(query, signal) {
  ...
  try {
    ...
    // 1. Search for matching symbols via Edge function
    const searchRes = await fetch(
      `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/yfinance-proxy?q=${encodeURIComponent(query)}`,
      {
        headers: { 'Authorization': `Bearer ${session?.access_token}` },
        signal
      }
    ...
    // 2. Fetch live prices via Edge function charting
    const p1 = Math.floor(Date.now() / 1000 - 86400 * 3)
    const p2 = Math.floor(Date.now() / 1000)
    ...
    const results = await Promise.all(matches.map(async (q) => {
      ...
      try {
        const res = await fetch(
          `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/yfinance-proxy?ticker=${encodeURIComponent(q.symbol)}&period1=${p1}&period2=${p2}`,
          {
            headers: { 'Authorization': `Bearer ${session?.access_token}` },
            signal
          }
...
export default function WatchlistSidebar() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      // Use functional state update to prevent stale closure of prices state
      setPrices(prevPrices => {
        const updatedPrices = { ...prevPrices }
        priceResults.forEach(({ sym, p }) => {
          if (p !== null) updatedPrices[sym] = p
        })

        // Check alerts against updated prices
        const alertHits = items.filter(item => {
          if (!item.is_active || !updatedPrices[item.symbol]) return false
          const currentPrice = updatedPrices[item.symbol]
          const target = Number(item.target_price)
          return (item.target_type === 'above' && currentPrice >= target) ||
                 (item.target_type === 'below' && currentPrice <= target)
        })

        if (alertHits.length > 0) {
          alertHits.forEach(item => {
            const currentPrice = updatedPrices[item.symbol]
            const target = Number(item.target_price)
            toast.custom((t) => (
          ...
          // Disable fired alerts in DB in one batch
          Promise.all(
            alertHits.map(item => supabase.from('watchlists').update({ is_active: false }).eq('id', item.id))
          ).then(() => {
            if (isMounted) loadWatchlist()
          })
        }
